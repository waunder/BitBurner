...
**Continuous single-target workers (September 15).** The normal MCP path launches each action without `once`, so workers loop on their assigned action while `mcp.js` continues reassessing on `home` every tick. This removes the normal completion-and-relaunch gap. A material target or allocation change still uses the established tolerance/age rule, releases all changed action RAM first, then launches weaken/grow/hack replacements in that order. Missing complementary jobs may use currently free RAM without disturbing an in-flight action. The separately controlled cloud multi-target scheduler continues to use finite `once` cohorts. A failed launch is recorded in `workerLaunchFailures` and raises the durable `workerLaunchSucceeded` invariant; it is retried through normal reconciliation. Remote running-script lookups name the worker host explicitly. Status exposes `workerMode: continuous` for the normal path and `finite` for cloud cohorts.

**Full-money handling (September 15).** The normal money and reputation path uses the same balanced hack/grow/maintenance-weaken allocation at full money as it does during restoration. Its network-wide hack budget remains `max(1, floor(0.10 / hackAnalyze(target)))`, approximately 10% of current money per allocation, with a one-thread minimum if one thread already exceeds that fraction. Retained hacks consume that budget before new hacks launch; the remainder of available capacity stays active in grow plus its maintenance weaken rather than sitting idle. XP retains its separate hack/grow weights and recovery grow for experience.
...
Call the action on `ns.args[0]`. Optional second argument `once` makes a worker exit after exactly one completed call; without it, the worker loops. Normal MCP launches looping workers so reassessment does not create a completion/relaunch gap. The cloud multi-target scheduler supplies `once` for its bounded cohorts. A target switch or manager restart can still retire workers immediately. Process arguments expose the worker mode.
...
Available/maximum money use compact decimal units with one decimal place (`K`, `M`, `G`, `T`, `P`), such as `24.8G`; seven-character columns expand rather than truncate larger values.
...
The consolidated HUD displays the resolved `mcp_status.json.config.OBJECTIVE` in its compact MCP row and as `Objective` in the expanded section, refreshing every five seconds. Its expanded MCP deployment line shows active action hosts / available worker hosts and their percentage, total active threads, MCP action-RAM utilization percentage, and weaken/grow/hack threads, so unused worker capacity is visible without inferring it from a target panel. In multi-target mode it reports only target counts by harvest/preparation phase; `get_target_stats.js` owns the detailed per-target money, security, and thread view. The consolidated HUD is rendered in white and expands to fit its longest line. Missing/invalid values show `unknown`; stale expanded status marks it `last reported`. Restart `hud_consolidated.js` after source changes (it closes/retires its previous instance).
