...
4. **R4 (scoring + ramp discount + switch factor) — not started.** Ship
   ...
   per §2's own note.
5. **R5 (per-script redeploy) — code shipped 2026-08-14, NOT YET CONFIRMED
   LIVE.** Built and unit-tested in an isolated worktree with no game
   connection (same verification limits as R1/R3 above — `node --test`/
...
6. **R7 (cheap items) — code shipped 2026-08-14, NOT YET CONFIRMED LIVE.**
   Same verification limits as above. All four bullets from §2's R7 section
   addressed:
...
shipped in code, but only R2 and R3 are confirmed live — R1 (the actual
order-of-magnitude payload) still needs an in-game restart and a live
observation window before it can be called done. R5 and R7 (steps 5-6) are
single-digit-percent items layered on top, shipped in code alongside R1 but
likewise not yet restarted/observed live.
