...
| `dnet_manager_registry.json` | `dnet_root.js` (folded from `dnet_manager_active_<host>.json` shards, cadence `REGISTRY_MERGE_MS`) | `{host: ts}` of currently-believed-active resident managers. A crawler reserves a slot before spawning its manager; the manager refreshes that shard every 15 seconds while phishing. Entries older than two minutes are dropped on the next home-side merge, so a killed/orphaned manager is restored promptly instead of blocking the registry for five minutes. |
