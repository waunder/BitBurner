...
| Event log | `mcp_events.txt`, transitions only, each carrying its predicate's inputs. Trimmed to 300 lines at startup; last 20 inline in the status. | **Shipped broken and unnoticed for its first day.** First named `mcp_events.jsonl`; Bitburner's `ns.write` only accepts `.txt`/`.json`/`.css`/a script extension, so every write threw and was silently swallowed by a try/catch into `ns.print`. The file never existed in the game — invisible because the in-memory ring buffer feeding `recentEvents` in the status kept working regardless. Found by checking why a correctly-set download pattern wasn't pulling the file down, not by any of the telemetry built to catch exactly this. Fixed by renaming to `.txt` and adding the `eventLogWrites` invariant so a repeat toasts instead of vanishing. Whether 300 lines and 20 inline are the right numbers is still open. |
| Invariants | Seven checks, toast once per name per run, counts in the status, `INVARIANT` verdict. | `poolNotIdle` at `>= 0.5` is a guess. If it fires constantly during legitimate weaken phases it is noise, and noise is worse than nothing. Retune or drop it. |
