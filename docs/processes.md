...
| `dnet_manager.js` | resident on darknet nodes | 3.15GB measured live | Local lifecycle manager. After the transient crawler releases RAM, it runs one-shot loot, then currently starts **one** `dnet_phish.js` worker as the conservative freeze-diagnosis profile. It immediately opens a generated cache after the phisher exits, and every 90 seconds lets the phisher stop cleanly before running a brief recrawl. No kill call or durable in-memory state is required. |
