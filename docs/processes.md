...
| `dnet_manager_registry.json` | `dnet_root.js` (folded from `dnet_manager_active_<host>.json` shards, same 5s loop as the credential merge) | **New 2026-08-30, the concurrency-cap fix.** `{host: ts}` of currently-believed-active resident managers. `dnet_crawl.js` reserves a slot here (writes+ships a shard) right before it would spawn `dnet_manager.js`, and `dnet_manager.js` itself refreshes the same shard once per main-loop iteration so a still-alive manager never goes stale (`MANAGER_STALE_MS`, 5min) and gets wrongly evicted from the count. Entries older than that are dropped on the next home-side merge — self-healing for a killed/orphaned manager, no explicit deregistration needed. Pushed to every newly-delegated host alongside `dnet_creds.txt` (same conditional-push pattern) so a freshly-landed `dnet_crawl.js` instance can read a real local snapshot before deciding whether to become resident. In `WATCHED_FILES`/`PULL_FILES` — watch its size directly after a restart; it should plateau at `MAX_ACTIVE_MANAGERS` (15) instead of growing unbounded. |
