...
The consolidated HUD displays the resolved `mcp_status.json.config.OBJECTIVE` in its compact MCP row and as `Objective` in the expanded section, refreshing every five seconds. Its expanded MCP deployment line shows active action hosts / available worker hosts and their percentage, total active threads, MCP action-RAM utilization percentage, and weaken/grow/hack threads, so unused worker capacity is visible without inferring it from a target panel. Missing/invalid values show `unknown`; stale expanded status marks it `last reported`. Restart `hud_consolidated.js` after source changes (it closes/retires its previous instance).
