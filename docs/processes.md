...
| `dnet_deploy.js` | `home`, then darknet | ~4.9GB | Roaming self-replicating controller. Cracks, persists, and follows mutations. **2026-08-14:** before placing permanent workloads, it launches temporary `dnet_realloc.js` on a blocked neighbour at the largest thread count that fits and waits for a later mutation pass to observe completion. Keeping the 1GB reallocation API out of every permanent crawler preserves one additional phishing thread on common 16GB hosts. Once prepared it spreads, launches one-shot loot once per neighbour/process lifetime (plus whenever a phishing cache marker appears), then fills remaining RAM with `dnet_phish.js`. Heartbeats include observed RAM recovery, preparation workers/threads, and phishing workers/threads. Estimate adds `ps` to the prior call set; confirm in-game. |
