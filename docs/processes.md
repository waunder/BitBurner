...
| `dnet_manager.js` | resident on darknet nodes | 3.15GB measured live | Local lifecycle manager. After the transient crawler releases RAM, it runs one-shot loot, fills all remaining capacity with `dnet_phish.js`, immediately opens a generated cache after the phisher exits, and every 90 seconds lets the phisher stop cleanly before running a brief recrawl. The first full recrawl cycle is live-confirmed. No kill call or durable in-memory state is required. |
...
| `dnet_phish.js` | a prepared darknet server | ~3.6GB/thread | Lean `phishingAttack()` loop deployed at the largest thread count that fits after crawl and loot setup. It deliberately owns no durable counters. On a cache-producing success it writes a zero-RAM marker and exits; the controller launches loot to open the cache, then restores phishing on a later pass. Other mutations may kill it at any time and a surviving crawler restores it. Aggregate effects are visible through `mcp_status.json` player charisma and the game's darknet money-source category. |
| `dnet_loot.js` | a darknet server | ~5.55GB | Frees blocked RAM (normally already reclaimed remotely by the new crawler path), opens `.cache` files, and reports karma spent. Meaningful results are now immutable timestamped event shards (`dnet_loot_<escaped-host>_<timestamp>.json`); no-op runs write nothing, so later passes cannot overwrite prior gains. Launched once per neighbour/process lifetime by `dnet_deploy.js`. |
...
| `dnet_manager_registry.json` | `dnet_root.js` (folded from `dnet_manager_active_<host>.json` shards, same poll loop as the credential merge, cadence `REGISTRY_MERGE_MS`) | **New 2026-08-30, the concurrency-cap fix.** `{host: ts}` of currently-believed-active resident managers. `dnet_crawl.js` reserves a slot here (writes+ships a shard) right before it would spawn `dnet_manager.js`, and `dnet_manager.js` itself refreshes the same shard once per main-loop iteration so a still-alive manager never goes stale (`MANAGER_STALE_MS`, 5min) and gets wrongly evicted from the count. Entries older than that are dropped on the next home-side merge — self-healing for a killed/orphaned manager, no explicit deregistration needed. Pushed to every newly-delegated host alongside `dnet_creds.txt` (same conditional-push pattern) so a freshly-landed `dnet_crawl.js` instance can read a real local snapshot before deciding whether to become resident. In `WATCHED_FILES`/`PULL_FILES` — watch its size after a restart. **This is a soft cap, not a hard ceiling**: the first live run overshot `MAX_ACTIVE_MANAGERS` (15 → 8 after that) to 30 entries before merge cadence caught up, and the *tightened* cap then overshot *worse* (36 entries, froze faster) on the next attempt — the actual driver turned out to be the propagation burst itself, not steady-state resident count, since the network is now mostly pre-cracked and re-propagates near-instantly. `MAX_SPREAD_PER_PASS` (`dnet_crawl.js`) and `jitteredRecrawlMs` (`dnet_manager.js`) throttle that burst directly as of the same day — see `docs/darknet-strategy.md`'s 2026-08-30 updates for the full incident (two live misses before this). |
