...
export async function reconcileReputation(ns, prior, { objective, hosts, reserveHomeGb, runId, emit }) {
  ...
  for (const p of owned) {
    if (!desired || Number(p.args[0]) !== ns.pid || p.args[1] !== signature || owned.length > 1 || !(scriptRam > 0) || p.threads * scriptRam > Math.min(config.ramGb, MAX_SHARE_RAM_GB)) {
      if (!ns.kill(p.pid, p.host)) throw new Error(`Could not retire reputation worker ${p.pid}`)
  ...
  const live = owned.find(p => Number(p.args[0]) === ns.pid && p.args[1] === signature && ns.isRunning(p.pid, p.host))
  ...
  for (const p of ns.ps(choice.host)) if (ACTIONS.has(p.filename.replace(/^\//, "")) && !ns.kill(p.pid, choice.host)) throw new Error(`Could not release worker ${p.pid}`)
