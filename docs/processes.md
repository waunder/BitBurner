...
| `dnet_deploy.js` | `home`, then darknet | measured live in `controllerRam.scriptRam` | Roaming self-replicating controller. Cracks, persists, and follows mutations. **2026-08-14:** before placing permanent workloads, it launches temporary `dnet_realloc.js` from the crawler toward a blocked direct neighbour at the largest thread count that fits and waits for a later mutation pass to observe completion. Keeping the 1GB reallocation API out of every permanent crawler preserves one additional phishing thread on common 16GB hosts. Once prepared it spreads, launches one-shot loot once per neighbour/process lifetime (plus whenever a phishing cache marker appears), then fills remaining RAM with `dnet_phish.js`. Heartbeats include observed RAM recovery, preparation/phishing workers and threads, the latest skip decision with every RAM input used by its predicate, and measured controller/max/used/free/blocked RAM plus the local process list. |
